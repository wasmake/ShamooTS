...
declare const MANAGED_LOBBY_FILES: readonly ["config.yml", "messages.yml", "items.yml", "menus.yml", "scoreboard.yml", "servers.yml", "spawn.yml", "portals.yml"];
type ManagedLobbyFile = (typeof MANAGED_LOBBY_FILES)[number];
type ManagedLobbyVisibilityMode = 'all' | 'none' | 'staff' | 'cycle';
/** Plain host data bounded to 32 levels, 100,000 values, 10,000 entries per collection, and 1 MiB per string. */
type ManagedLobbyData = null | boolean | number | string | readonly ManagedLobbyData[] | ManagedLobbyDataRecord;
interface ManagedLobbyDataRecord {
    readonly [key: string]: ManagedLobbyData;
}
interface ManagedLobbyEnsureRequest {
    readonly operation: 'ensure';
}
interface ManagedLobbyReadRequest {
    readonly operation: 'read';
    /** Omit to read all eight managed YAML files. */
    readonly file?: ManagedLobbyFile;
}
interface ManagedLobbyWriteRequest {
...
interface ManagedLobbyReloadRequest {
    readonly operation: 'reload';
}
interface ManagedLobbyStatusRequest {
    readonly operation: 'status';
}
type ManagedLobbyExecuteAction = {
    readonly action: 'setspawn';
    readonly player: string;
} | {
    readonly action: 'spawn';
    readonly player: string;
} | {
    readonly action: 'items';
    readonly player: string;
} | {
    ...
    readonly action: 'portal-wand';
    readonly player: string;
} | {
    readonly action: 'portal-pos1';
    readonly player: string;
} | {
    readonly action: 'portal-pos2';
    readonly player: string;
} | {
    ...
    readonly action: 'portal-remove';
    readonly player: string;
    readonly id: string;
} | {
    readonly action: 'portal-list';
} | {
    readonly action: 'portal-info';
    readonly id: string;
} | {
    readonly action: 'portal-enable';
    readonly player: string;
    readonly id: string;
} | {
    readonly action: 'portal-disable';
    readonly player: string;
    readonly id: string;
} | {
    ...
    readonly action: 'portal-visualize';
    readonly player: string;
    readonly enabled: boolean;
};
type ManagedLobbyExecuteRequest = {
    readonly operation: 'execute';
} & ManagedLobbyExecuteAction;
type ManagedLobbyRequest = ManagedLobbyEnsureRequest | ManagedLobbyReadRequest | ManagedLobbyWriteRequest | ManagedLobbyReloadRequest | ManagedLobbyStatusRequest | ManagedLobbyExecuteRequest;
type ManagedLobbySuccess = ManagedLobbyDataRecord & {
    readonly ok: true;
    readonly state: string;
    readonly error?: never;
};
type ManagedLobbyFailure = ManagedLobbyDataRecord & {
    readonly ok: false;
    readonly state: string;
 ...
 */
type ManagedLobbyResult = ManagedLobbySuccess | ManagedLobbyFailure;
interface PaperManagedLobbyHost {
    paperManagedLobby(request: ManagedLobbyRequest): Promise<unknown>;
}
declare class ManagedLobbyUnavailableError extends Error {
    readonly code = "MANAGED_LOBBY_UNAVAILABLE";
    constructor(message: string, options?: ErrorOptions);
}
declare class ManagedLobbyProtocolError extends Error {
    readonly code = "MANAGED_LOBBY_PROTOCOL_ERROR";
    constructor(message: string, options?: ErrorOptions);
}
/**
 ...
 */
declare function paperManagedLobby(request: ManagedLobbyRequest): Promise<ManagedLobbyResult>;
...
export { type ActionDescriptor, type ClickActionDescriptor, type InventoryDescriptor, type InventoryOptions, type InventorySlotDescriptor, type ItemActionDescriptor, type ItemDescriptor, type ItemOptions, type LegacyTextDescriptor, MANAGED_LOBBY_FILES, type ManagedLobbyData, type ManagedLobbyDataRecord, type ManagedLobbyEnsureRequest, type ManagedLobbyExecuteAction, type ManagedLobbyExecuteRequest, type ManagedLobbyFailure, type ManagedLobbyFile, ManagedLobbyProtocolError, type ManagedLobbyReadRequest, type ManagedLobbyReloadRequest, type ManagedLobbyRequest, type ManagedLobbyResult, type ManagedLobbyStatusRequest, type ManagedLobbySuccess, ManagedLobbyUnavailableError, type ManagedLobbyVisibilityMode, type ManagedLobbyWriteRequest, type MiniMessageDescriptor, type MiniMessageOptions, type MutablePaperEvent, PAPER_VELOCITY_MESSAGE_BRIDGE, PAPER_VELOCITY_TRANSPORT, type PaperAction, type PaperActionCallback, type PaperActionContext, type PaperCommandArgumentDescriptor, type PaperCommandContext, type PaperCommandItem, type PaperCommandOptionDescriptor, type PaperCommandPlayer, type PaperCommandRegistry, type PaperCommandResult, type PaperCommandRouteDescriptor, type PaperCommandSender, type PaperEntrypoint, type PaperEventContext, type PaperEventHandler, type PaperEventRegistry, type PaperEventType, type PaperHostApi, type PaperManagedLobbyHost, type PaperMessagingChannel, type PaperMessenger, type PaperProxyResponse, type PaperRuntimeHost, type PaperScheduler, type PaperVelocityMessageBridge, PaperVelocityTransport, type RegisteredPaperEvent, type ScheduledHandle, type TextDescriptor, type TextLike, type TextOptions, type TextStyle, VelocityTransportUnavailableError, createPaperHostApi, createPaperPlatform, definePaperEntrypoint, inventory, item, legacyText, miniMessage, paperCommunicationProviders, paperHostCommunicationProviders, paperManagedLobby, text };
```
